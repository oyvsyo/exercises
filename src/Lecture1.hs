...
strSum str = sum (map readInt (words str))
...
cnt_elems operator x elements = show (foldr (\element counter -> counter + (fromEnum (operator element x))) 0 elements)

lowerAndGreater n list = show n ++ " is greater than " ++ cnt_elems (<) n list ++ " elements and lower than " ++ cnt_elems (>) n list ++ " elements"
